...
TABLE_DDL = [
    ...
    "                CREATE TABLE IF NOT EXISTS recommendation_version_evidence_links (\n                    link_id TEXT PRIMARY KEY,\n                    recommendation_version_id TEXT NOT NULL REFERENCES recommendation_versions(recommendation_version_id) ON UPDATE CASCADE,\n                    evidence_id TEXT NOT NULL REFERENCES evidence_items(evidence_id) ON UPDATE CASCADE,\n                    recommendation_candidate_id TEXT REFERENCES recommendation_candidates(candidate_id) ON UPDATE CASCADE,\n                    pico_id TEXT REFERENCES pico_questions(pico_id) ON UPDATE CASCADE,\n                    link_reason TEXT NOT NULL DEFAULT 'release_linked_evidence',\n                    link_status TEXT NOT NULL DEFAULT 'publish_ready',\n                    normalized_payload JSONB NOT NULL DEFAULT '{}'::jsonb,\n                    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,\n                    UNIQUE (recommendation_version_id, evidence_id)\n                )\n                ",
...
CONSTRAINT_MIGRATIONS = [
    ...
    _add_check_constraint(
        ...
        "ck_recommendation_version_evidence_links_status",
        f"link_status IN ({_sql_literal_list(('publish_ready', 'review_backlog', 'removed'))})",
    ),
    _add_check_constraint(
        "update_logs",
