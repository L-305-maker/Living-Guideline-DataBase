...
def main() -> None:
    ...
    for doc_blocks in group_by_doc(blocks).values():
        chunks.extend(build_atomic_chunks(doc_blocks, sections_by_doc.get(doc_blocks[0].doc_id, []), meta_from_block(doc_blocks[0]), warnings))
    ...
    print(f"wrote {len(chunks)} atomic chunks to {args.output}")
